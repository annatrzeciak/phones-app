...
console.log(this.searchPhoneForm.value[filter]);
    ...
    this.data.memory=[];
    this.data.battery=[];
//     this.data.resolution=this.data.resolution.map(res=>{
// return {name:res.name,checked:res.checked,count:0};
//     }
//     );
   console.log(JSON.stringify(this.filters))
    ...
    .then(result => {
      this.phones = Object.values(result).map(result => {
        ...
        if (!isNaN(array.screen_diagonal)) this.data.diagonal.push(array.screen_diagonal);
        ...
        if (filter === "first"){
          ...
          var resolution = this.data.resolution.find(x=>x['name']==array.screen_resolution) ;
        
          if (resolution==undefined && array.screen_resolution!=''  ){
          this.data.resolution.push( {name: array.screen_resolution, checked: this.searchPhoneForm.value.resolution.indexOf(array.screen_resolution)==-1? false : true ,count:1} )
                   }else if(array.screen_resolution!=''){
                    resolution = {name:  array.screen_resolution,checked: this.searchPhoneForm.value.resolution.indexOf(array.screen_resolution)==-1? false : true  ,count:resolution.count ++  };
        }
      ...
      this.data.diagonalRange = [this.data.diagonal[0],this.data.diagonal[this.data.diagonal.length - 1]];
      this.data.RAM.sort((a, b) => a - b);
       this.data.RAMRange = [this.data.RAM[0],this.data.RAM[this.data.RAM.length - 1]];
      this.data.memory.sort((a, b) => a - b);
      this.data.memoryRange = [this.data.memory[0],this.data.memory[this.data.memory.length - 1]];
      this.data.battery.sort((a, b) => a - b);
      this.data.batteryRange = [this.data.battery[0],this.data.battery[this.data.battery.length - 1]];
      this.data.resolution.sort((a, b) =>   b.count - a.count)
      // this.searchPhoneForm.controls.resolution.setValue(this.data.resolution);
     

      if (filter === "first"){
      this.diagonalSlider.slider.updateOptions({range: {min: this.data.diagonal[0],max: this.data.diagonal[this.data.diagonal.length - 1]}});
      this.RAMSlider.slider.updateOptions({range: {min: this.data.RAM[0],max: this.data.RAM[this.data.RAM.length - 1]}});
      this.memorySlider.slider.updateOptions({range: {min: this.data.memory[0],max: this.data.memory[this.data.memory.length - 1]}});
      this.batterySlider.slider.updateOptions({range: {min: this.data.battery[0],max: this.data.battery[this.data.battery.length - 1]}});
      // console.log(this.searchPhoneForm);
    console.log(this.data.resolution);
  ...
  onChange(res:string, isChecked: boolean) {
    ...
    if(isChecked) {
      ...
      console.log(resolutionArray.controls)
